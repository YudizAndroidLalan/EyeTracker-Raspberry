...
def get_darkest_area(image):
    ...
    ignoreBounds = 30 #OPTIMIZED: increased from 20 to 30 to avoid edge artifacts
    imageSkipSize = 8 #OPTIMIZED: reduced from 10 to 8 for better sampling density
    searchArea = 25 #OPTIMIZED: increased from 20 to 25 for better local analysis
    internalSkipSize = 3 #OPTIMIZED: reduced from 5 to 3 for finer sampling within search area
...
def optimize_contours_by_angle(contours, image):
    ...
    # OPTIMIZED: dynamic spacing based on contour size for better point distribution
    spacing = max(5, int(len(all_contours)/30))  # minimum spacing of 5, max ratio of 1/30
    ...
    for i in range(0, len(all_contours), 1):
        ...
        # OPTIMIZED: relaxed angle threshold from 60 to 75 degrees for better contour retention
        cos_threshold = np.cos(np.radians(75))  # Convert angle to radians
...
def check_contour_pixels(contour, image_shape, debug_mode_on):
    ...
    # OPTIMIZED: adjusted ellipse thickness for better coverage analysis
    cv2.ellipse(ellipse_mask_thick, ellipse, (255), 8) # reduced from 10 to 8 for tighter fit
    cv2.ellipse(ellipse_mask_thin, ellipse, (255), 3) # reduced from 4 to 3 for more precise matching
...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, render_cv_window):
    ...
    # OPTIMIZED: morphological operations for better contour extraction
    kernel_size = 3  # reduced from 5 to 3 for more precise morphology
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
    # Add closing operation to fill gaps in pupil
    ...
    for i in range(1,4):
        # OPTIMIZED: improved morphological operations
        # First apply closing to connect nearby regions
        closed_image = cv2.morphologyEx(image_array[i-1], cv2.MORPH_CLOSE, closing_kernel)
        # Then dilate with smaller kernel for final refinement
        dilated_image = cv2.dilate(closed_image, kernel, iterations=1) # reduced from 2 to 1
        ...
        # OPTIMIZED: more lenient contour filtering
        reduced_contours = filter_contours_by_area_and_return_largest(contours, 800, 3.5) # reduced area from 1000 to 800, increased ratio from 3 to 3.5
        ...
        if len(reduced_contours) > 0 and len(reduced_contours[0]) > 5:
            ...
            # OPTIMIZED: improved scoring function that balances all factors
            final_goodness = (current_goodness[0] ** 0.8) * (total_pixels[0] ** 0.3) * (total_pixels[1] ** 1.2)
    ...
    if render_cv_window:
        cv2.imshow('Eye Tracking - Webcam Feed', test_frame)
...
def process_frame(frame):
    ...
    # OPTIMIZED: adjusted threshold values for better pupil detection
    # Tighter thresholds for better pupil isolation
    thresholded_image_strict = apply_binary_threshold(gray_frame, darkest_pixel_value, 8)  # increased from 5 to 8
    thresholded_image_strict = mask_outside_square(thresholded_image_strict, darkest_point, 200)  # reduced from 250 to 200

    thresholded_image_medium = apply_binary_threshold(gray_frame, darkest_pixel_value, 18)  # increased from 15 to 18
    thresholded_image_medium = mask_outside_square(thresholded_image_medium, darkest_point, 220)  # reduced from 250 to 220
    
    thresholded_image_relaxed = apply_binary_threshold(gray_frame, darkest_pixel_value, 28)  # increased from 25 to 28
    thresholded_image_relaxed = mask_outside_square(thresholded_image_relaxed, darkest_point, 240)  # reduced from 250 to 240
...
# Start webcam eye tracking with Raspberry Pi camera support
def start_webcam_tracking():
    print("Starting webcam eye tracking...")
    print("Controls:")
    print("  SPACE = pause/resume")
    print("  D = toggle debug mode")
    print("  Q = quit")
    
    # Try different methods to access Raspberry Pi camera
    cap = None
    camera_backends = [
        (cv2.CAP_V4L2, "V4L2"),
    ...
    # Try libcamera (newer Raspberry Pi OS)
    if cap is None:
        try:
            ...
            if result.returncode == 0:
                print("Libcamera detected. You may need to use libcamera-vid instead.")
                print("Try running: libcamera-vid -t 0 --width 640 --height 480 --framerate 30")
        except:
            pass
    ...
    if cap is None or not cap.isOpened():
        ...
        print("2. Check camera connection and ribbon cable")
        print("3. Try: vcgencmd get_camera")
        print("4. For newer Raspberry Pi OS, try: libcamera-hello --list-cameras")
        print("5. Reboot your Raspberry Pi")
        return
    ...
    # Try to set exposure (this may not work on all cameras)
    try:
    ...
    print("Webcam feed started. Position your eye in front of the camera.")
    ...
    while True:
        if not paused:
            ...
            if debug_mode_on:
                ...
                cv2.imshow('Darkest Point Detection', darkest_image)
            ...
            thresholded_image_strict = mask_outside_square(thresholded_image_strict, darkest_point, 200)

            thresholded_image_medium = apply_binary_threshold(gray_frame, darkest_pixel_value, 18)
            thresholded_image_medium = mask_outside_square(thresholded_image_medium, darkest_point, 220)
            
            thresholded_image_relaxed = apply_binary_threshold(gray_frame, darkest_pixel_value, 28)
            thresholded_image_relaxed = mask_outside_square(thresholded_image_relaxed, darkest_point, 240)
            
            # Process the frames to find the pupil
            ...
            # Print pupil center coordinates if detected
            if pupil_rotated_rect[0] != (0, 0):
                center_x, center_y = pupil_rotated_rect[0]
                print(f"Pupil center: ({center_x:.1f}, {center_y:.1f})")
    ...
    cap.release()
    cv2.destroyAllWindows()
    print("Webcam eye tracking stopped.")
